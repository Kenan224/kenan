...
def main():
        ...
        if uploaded_file is not None:
            try:
                ...
                if auto_a0_calculated:
                    ...
                    st.info("Вы можете отредактировать загруженные данные перед анализом. Первое значение в столбце А будет автоматически использовано как начальная оптическая плотность А0 для всех расчетов. Столбец А/А0 будет пересчитан автоматически.")
                ...
                column_config = {
                    ...
                    "А": st.column_config.NumberColumn(
                        ...
                        help="Оптическая плотность А (первое значение используется как А0)",
                        min_value=0.0,
                        step=0.0001,
                        format="%.5f"
                    )
    ...
    if df is not None and not df.empty:
        ...
        with col3:
            st.markdown(f"""
            <div class="summary-stat">
                <div class="metric-label">🧪 Начальная Оптическая плотность</div>
