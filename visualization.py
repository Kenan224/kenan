...
def create_matplotlib_plots(df: pd.DataFrame, selected_data: pd.DataFrame,
                           zo_predictions: pd.DataFrame, pfo_predictions: pd.DataFrame,
                           pso_predictions: pd.DataFrame,
    ...
    # ZO plot
    ax0.plot(df['т, мин'], df['А'], 'mo-', label='Экспериментальные данные (A)', markersize=4)
    ax0.plot(zo_predictions['т, мин'], zo_predictions['ZO_pred'], 'r--',
             label=f'Модель ZO (k₀={abs(k0):.5f})', linewidth=2)
    ax0.set_title('Модель нулевого порядка')
